...
import AksesHPBot from './hpbot/AksesHPBot';
import EditHPBot from './hpbot/EditHPBot';
const { height, width } = Dimensions.get('window');

const lpOps = [
  {
    id: 1,
    name: 'Akses Halaman Data Hasil Pemeriksaan',
  },
  {
    id: 2,
    name: 'Edit Data Hasil Pemeriksaan',
  },
];

function HPBot({ disabled }) {
    ...
    return (
      <View style={{ marginVertical: 5, paddingVertical: 7, paddingHorizontal: 10, backgroundColor: Colors.PRIMARY_PURPLE, maxWidth: 250, borderRadius: 7 }}>
        <View style={{ marginBottom: 7 }}>
          <Text style={{ color: Colors.DEFAULT_WHITE, fontWeight: 'bold' }}>Data Hasil Pemeriksaan merupakan data yang diperoleh dari kader dalam memeriksa kondisi tiap anak untuk mengklasifikasi anak berdasarkan statusnya.</Text>
        </View>
  ...
  return (
    <View>
      ...
      {lp2[0] !== undefined && lp[0] === 'akses halaman data hasil pemeriksaan' ? <AksesHPBot /> : lp2[0] !== undefined && lp[0] === 'edit data hasil pemeriksaan' ? <EditHPBot /> : null}
      {lp3[0] !== undefined ? lpOpss() : null}

      {lp[1] !== undefined ? userlp(1) : null}
      {lp2[1] !== undefined && lp[1] === 'akses halaman data hasil pemeriksaan' ? <AksesHPBot /> : lp2[1] !== undefined && lp[1] === 'edit data hasil pemeriksaan' ? <EditHPBot /> : null}
      {lp3[1] !== undefined ? lpOpss() : null}

      {lp[2] !== undefined ? userlp(2) : null}
      {lp2[2] !== undefined && lp[2] === 'akses halaman data hasil pemeriksaan' ? <AksesHPBot /> : lp2[2] !== undefined && lp[2] === 'edit data hasil pemeriksaan' ? <EditHPBot /> : null}
      {lp3[2] !== undefined ? lpOpss() : null}

      {lp[3] !== undefined ? userlp(3) : null}
      {lp2[3] !== undefined && lp[3] === 'akses halaman data hasil pemeriksaan' ? <AksesHPBot /> : lp2[3] !== undefined && lp[3] === 'edit data hasil pemeriksaan' ? <EditHPBot /> : null}
      {lp3[3] !== undefined ? lpOpss() : null}
    </View>
  );
}
export default HPBot;

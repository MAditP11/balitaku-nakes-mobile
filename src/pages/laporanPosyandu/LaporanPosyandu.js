...
import { getReport } from '../../apis/ReportPosyandu';
import TopBar from '../../components/TopBar';
import ButtonBack from '../../components/ButtonBack';
import { Ionicons } from '@expo/vector-icons';
const { height, width } = Dimensions.get('window');

function LaporanPosyandu({ navigation }) {
  ...
  const getReportImage = (value) => {
    ...
    if (value) {
      ...
      return <Ionicons name="document-text" size={30}></Ionicons>;
    }
  };
  ...
  return (
    <View style={{ flex: 1, alignItems: 'center' }}>
      ...
      <TopBar height={height / 7} content={<ButtonBack show={true} title={'Laporan Posyandu'} />} />
      ...
      <TouchableOpacity
        activeOpacity={0.8}
        onPress={() => navigation.navigate('AddLaporanPosyandu')}
        style={{
      ...
      <View height={height * 1} style={{ alignItems: 'center' }}>
        {report[0] === undefined ? (
          ...
          <View height={height / 1.25} width={width}>
            <FlatList
              data={report}
                ...
                return (
                  <View style={{ alignItems: 'center' }}>
                    <View width={width / 1.1}>
                      <TouchableOpacity activeOpacity={0.8} onPress={() => navigation.navigate('DetailLaporanPosyandu', { data: item.item })} style={{ flexDirection: 'row', marginVertical: 10 }}>
                        <View style={{ alignItems: 'center', justifyContent: 'center', width: '20%' }}>{getReportImage(item.item.image_posyandu[0].uri)}</View>

...
import { Dimensions, RefreshControl, TouchableOpacity, Image, StatusBar, StyleSheet, Text, View, ScrollView } from 'react-native';
import { deleteReport } from '../../apis/ReportPosyandu';
...
function DetailLaporanPosyandu({ navigation, route }) {
  ...
  const getReportImage = (value) => {
    if (value) {
      let uris = axios.defaults.baseURL + '/images/report/nakes/posyandu/' + value;
  ...
  return (
    <View style={styles.container}>
      ...
      <TopBar height={height / 7} content={<ButtonBack title={'Detail Laporan Posyandu'} />} />
      ...
      <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />} height={height * 2} width={width}>
        <View style={{ alignItems: 'center' }}>
          <View width={width / 1.1} style={styles.editProfil}>
            {/* Top content */}
            <View style={{ alignItems: 'center', borderBottomColor: Colors.grey, paddingBottom: 5, borderBottomWidth: 1, borderStyle: 'dashed' }}>
              <Text style={{ fontSize: 16, fontWeight: 'bold' }}>{data.name_activity ? data.name_activity : '-'}</Text>
              <Text style={{ marginBottom: 3 }}>{me.puskesmas ? me.puskesmas.name : '-'}</Text>
              <View style={{ flexDirection: 'row', justifyContent: 'center', width: '90%' }}>
                <Text style={{ borderRightWidth: 1, paddingRight: 5, marginLeft: -4 }}>{me.puskesmas ? 'Kec. ' + me.puskesmas.district : '-'}</Text>
                <Text style={{ paddingLeft: 5 }}>{me.puskesmas ? 'Kel. ' + me.puskesmas.village : '-'}</Text>
              </View>
            </View>
            <View style={styles.profilCon}>
              ...
              <View style={{ marginTop: 15 }}>
                ...
                <View style={{ marginLeft: 15 }}>
                  <Text style={{ textAlign: 'justify', marginTop: 5 }}>{data.participant ? data.participant : '-'}</Text>
                </View>
              </View>
              ...
              <View>
                ...
                {/* {getReportImage(data.image_posyandu[0].uri)} */}
                {data.image_posyandu ? (
                  data.image_posyandu.map((item, key) => {
                    return (
          ...
          <View width={width / 1.15} style={{ marginVertical: 20, flexDirection: 'row', justifyContent: 'space-around' }}>
            ...
            <TouchableOpacity
              activeOpacity={0.8}
              onPress={() => navigation.navigate('EditLaporanPosyandu', { dataa: data })}
...
export default DetailLaporanPosyandu;

...
import { setReport } from '../../apis/ReportPosyandu';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { Dimensions, TouchableOpacity, ScrollView, StatusBar, StyleSheet, Image, Modal, Text, TextInput, View, Alert } from 'react-native';
...
function AddLaporanPosyandu({ navigation }) {
  ...
  const uploadImage = async (img, id) => {
    await AsyncStorage.getItem('token').then(async (token) => {
      img.forEach(async (element) => {
        ...
        const link = `${url}/nakes/laporan/upload/${id}`;
  ...
  const submit = async () => {
    ...
    navigation.navigate('LaporanPosyandu');
  };
      ...
      <ScrollView height={height} width={width / 1.15} style={styles.scrollCon}>
        ...
        <View>
          ...
          <TouchableOpacity onPress={() => showTimepicker()} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            <TextInput
              placeholder={'selected:  ' + textDate.toLocaleString().substring(10, 15)}
              editable={false}
              style={{ paddingHorizontal: 10, borderRadius: 5, height: height / 17, width: '50%', fontSize: 14, fontFamily: 'Poppins_600SemiBold', borderWidth: 1 }}
            />
          </TouchableOpacity>
        </View>
        ...
        <View style={{}}>
          ...
          <TextInput height={height / 10} multiline={true} placeholder="Masukkan peserta kegiatan" onChangeText={(text) => setParticipant(text)} style={styles.detailInput} />
        </View>
...
export default AddLaporanPosyandu;
